using Scanner.Protocol.Buffers;
...
namespace Scanner.Protocol.MessagePack
{
    /// <summary>
    /// Scanner消息读取器
    /// </summary>
    public ref struct ScannerMessagePackReader
    {
        /// <summary>
        /// 读取buffer
        /// </summary>
        ...
        private static readonly byte[] decodeDBDD = new byte[] { 0xDB, 0xDD };
        private static readonly byte[] decodeDBDE = new byte[] { 0xDB, 0xDE };
        private static readonly byte[] decodeDBDF = new byte[] { 0xDB, 0xDF };

        /// <summary>
        /// 解码（转义还原）,计算校验和
        /// </summary>
        /// <param name="srcBuffer"></param>
        public ScannerMessagePackReader(ReadOnlySpan<byte> srcBuffer)
        {
            SrcBuffer = srcBuffer;
            ReaderCount = 0;
        ...
        public void Decode(Span<byte> allocateBuffer)
        {
            ...
            int checkCodeLen = 1;
            if (checkCodeBufferSpan.SequenceEqual(decodeDBDD))
            {
                _realCheckXorCode = 0xDB;
                checkCodeLen += 1;
            }
            else if (checkCodeBufferSpan.SequenceEqual(decodeDBDE))
            {
                _realCheckXorCode = 0xD0;
                checkCodeLen += 1;
            }
            else if (checkCodeBufferSpan.SequenceEqual(decodeDBDF))
            {
                _realCheckXorCode = 0xD1;
                checkCodeLen += 1;
            }
            else
            {
            ...
            while (i < len)
            {
                if (tmpBufferSpan[i] == 0xDB)
                {
                    if (len > i + 1)
                    {
                        if (tmpBufferSpan[i + 1] == 0xDE)
                        {
                            allocateBuffer[offset++] = 0xD0;
                            _calculateCheckXorCode = (byte)(_calculateCheckXorCode ^ 0xD0);
                            i++;
                        }
                        else if (tmpBufferSpan[i + 1] == 0xDF)
                        {
                            allocateBuffer[offset++] = 0xD1;
                            _calculateCheckXorCode = (byte)(_calculateCheckXorCode ^ 0xD1);
                            i++;
                        }
                        else if (tmpBufferSpan[i + 1] == 0xDD)
                        {
                            allocateBuffer[offset++] = 0xDB;
                            _calculateCheckXorCode = (byte)(_calculateCheckXorCode ^ 0xDB);
                            i++;
                        }
                        else
                        {
        ...
        {
            var readOnlySpan = GetReadOnlySpan(len);
            string value = ScannerConstants.Encoding.GetString(readOnlySpan.Slice(0, len).ToArray());
            return value.Trim('\0');
        }
        /// <summary>
        /// 读取ASCII编码
        ...
        public DateTime ReadDateTime6(string format = "X2")
        {
            ...
            int year = BcdToInt(readOnlySpan[0])  + ScannerConstants.DateLimitYear;
        ...
        public DateTime? ReadDateTimeNull6(string format = "X2")
        {
            ...
            d = new DateTime(year + ScannerConstants.DateLimitYear, month, day, hour, minute, second);
            return d;
        }
        /// <summary>
        /// 读取可空类型的六字节日期,yyMMddHHmmss
        ...
        public DateTime ReadDateTime3(string format = "X2")
        {
            ...
            d = new DateTime(
            BcdToInt(readOnlySpan[0]) + ScannerConstants.DateLimitYear,
            BcdToInt(readOnlySpan[1]),
            BcdToInt(readOnlySpan[2]));
            return d;
        }
        ...
        public DateTime? ReadDateTimeNull3(string format = "X2")
        {
            ...
            if (year == 0 && month == 0 && day == 0) return null;
            d = new DateTime(
                year + ScannerConstants.DateLimitYear, month,day
            );
            return d;
        }
        /// <summary>
        ...
        public DateTime ReadUTCDateTime()
        {
            ...
            d = ScannerConstants.UTCBaseTime.AddSeconds(result).AddHours(8);
            return d;
        }
        /// <summary>
        /// 读取BCD编码
        ...
        public string ReadStringEndChar0()
        {
            ...
            string value = ScannerConstants.Encoding.GetString(ReadArray(length).ToArray());
            return value.Trim('\0');
        }
        /// <summary>
        ...
        public string ReadVirtualStringEndChar0()
        {
            ...
            string value = ScannerConstants.Encoding.GetString(GetVirtualReadOnlySpan(remainSpans.IndexOf((byte)'\0') + 1).ToArray());
            return value.Trim('\0');
        }
        /// <summary>
        /// 读取剩余数据体内容长度
