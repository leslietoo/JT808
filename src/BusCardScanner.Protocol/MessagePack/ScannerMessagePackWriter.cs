...
namespace Scanner.Protocol.MessagePack
{
    ...
    /// Scanner消息写入器
    /// </summary>
    public ref struct ScannerMessagePackWriter
    {
        private ScannerBufferWriter writer;

        /// <summary>
        /// 
        /// </summary>
        /// <param name="buffer">内存块</param>
        public ScannerMessagePackWriter(Span<byte> buffer)
        {
            this.writer = new ScannerBufferWriter(buffer);
        }
        /// <summary>
        /// 编码后的数组
        /// </summary>
        ...
        /// 写入头标识
        /// </summary>
        public void WriteStart()=> WriteByte(ScannerPackage.BeginFlag);
        /// <summary>
        /// 写入尾标识
        /// </summary>
        public void WriteEnd() => WriteByte(ScannerPackage.EndFlag);
        /// <summary>
        /// 写入空标识,0x00
        /// </summary>
        ...
        {
            byte[] codeBytes = ScannerConstants.Encoding.GetBytes(value);
        ...
        {
            Span<byte> codeBytes = ScannerConstants.Encoding.GetBytes(value);
        ...
        {
            var span = writer.Free;
            int yy = value.Year - ScannerConstants.DateLimitYear;
            span[0] = IntToBcd(yy);
        ...
        public void WriteDateTime6(in DateTime? value, in int fromBase = 16)
        {
            ...
            {
                int yy = value.Value.Year - ScannerConstants.DateLimitYear;
                span[0] = IntToBcd(yy);
        ...
        {
            ulong totalSecends = (ulong)(value.AddHours(-8) - ScannerConstants.UTCBaseTime).TotalSeconds;
            var span = writer.Free;
            //高位在前
            for (int i = 7; i >= 0; i--)
            {
        ...
        public void WriteFullEncode()
        {
            ...
            for (int i = 0; i < tmpSpan.Length; i++)
            {
                if (tmpSpan[i] == 0xDB)
                {
                    spanFree[tempOffset++] = 0xDB;
                    spanFree[tempOffset++] = 0xDD;
                }
                else if (tmpSpan[i] == 0xD0)
                {
                    spanFree[tempOffset++] = 0xDB;
                    spanFree[tempOffset++] = 0xDE;
                }
                else if (tmpSpan[i] == 0xD1)
                {
                    spanFree[tempOffset++] = 0xDB;
                    spanFree[tempOffset++] = 0xDF;
                }
                else
                {
